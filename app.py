...
if not st.session_state.logged:
    st.markdown('<div style="height:8vh"></div><div class="top" style="text-align:center"><div style="font-size:70px">🧬</div><div class="logo">VitaminCare</div><p class="muted">Your AI-powered nutrition screening companion</p></div>',unsafe_allow_html=True)
    st.markdown('<div class="card"><h2>Welcome back 👋</h2><p class="muted">Sign in to access your health dashboard and screening reports.</p></div>',unsafe_allow_html=True)
    name=st.text_input("Full name",placeholder="Enter your name")
    email=st.text_input("Email",placeholder="you@example.com")
    if st.button("Continue",type="primary",use_container_width=True):
        st.session_state.logged=True; st.session_state.name=name or "User"; go("Home"); st.rerun()
    st.caption("Academic prototype • Not a medical diagnostic device")
    st.stop()
