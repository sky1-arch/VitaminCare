...
.block-container{max-width:1120px;padding:0 1.2rem 6rem;color:#1f2937}
.top{background:linear-gradient(135deg,#dff7ed,#e8efff);padding:30px 24px;border-radius:0 0 30px 30px;margin-bottom:18px}
.logo{font-size:30px;font-weight:800;color:#172033}.muted{color:#667085}
...
/* Force readable text when Streamlit is using a dark theme with our light UI */
.stApp, .stApp p, .stApp label, .stApp h1, .stApp h2, .stApp h3,
.stApp h4, .stApp h5, .stApp h6, .stApp [data-testid="stMarkdownContainer"] {
    color: #1f2937;
}
.stApp .stCaption, .stApp small { color: #667085 !important; }
...
if not st.session_state.logged:
    ...
    .brand-mark {
        ...
        margin-bottom: 30px;
        background:
            radial-gradient(circle at 75% 15%, #0d6662 0 15%, transparent 40%),
            linear-gradient(145deg, #376e63 0%, #1e675f 38%, #dda238 72%, #e3a43b 100%);
    ...
    div[data-testid="stButton"] > button[kind="primary"] {
        background: #dfa039 !important;
        color: #071b42 !important;
        border-color: #dfa039 !important;
    }
    ...
    /* Make Streamlit form labels and inputs match the cream design. */
    .stApp label,
    .stApp [data-testid="stWidgetLabel"] p,
    .stApp [data-testid="stWidgetLabel"] label {
        color: #071b42 !important;
    ...
    # Page 1: landing screen only
    if "auth_mode" not in st.session_state:
        st.markdown("""
        <div class="auth-page">
            <div class="brand-mark"></div>
            <div class="auth-title">VitaminCare</div>
            <div class="auth-subtitle">
                Create an account or log in to save your screenings and track
                them over time.
            </div>
        """, unsafe_allow_html=True)
        ...
        st.markdown("""
            <div class="auth-footer">
                Your data stays tied to your own account.
            </div>
        </div>
        """, unsafe_allow_html=True)
    ...
    # Page 2: Login OR Page 3: Create account
    ...
    if mode == "Log in":
        st.markdown(
            ...
            '<div class="form-subtitle">Welcome back to VitaminCare.</div>',
            unsafe_allow_html=True,
        )
    else:
        st.markdown(
            ...
            'Start saving your vitamin screenings and track your progress.'
            '</div>',
            unsafe_allow_html=True,
        )
